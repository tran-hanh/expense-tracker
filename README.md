...
Streamlit is trying to write to your home directory. Run with the script above (it sets `HOME` to the project so Streamlit uses `.streamlit/` inside the repo), or run this from the project root: `HOME=$(pwd) streamlit run app.py`.
...
- **Filtering rules**:
  ...
  - **Month-specific exclusions**: Configurable per month in `src/core/filter_rules.py` (Dec 2025, Jan 2026, Feb 2026).
- **Custom exclusions**: Sidebar text input — comma-separated keywords or exact amounts — applied without code changes.
- **Interactive table**: "Valid Expenses" table has a "Count as Expense" checkbox per row; uncheck to exclude from Total Monthly Expense (KPIs update when you change checkboxes).
...
```python
(2026, 3): [
    "KEYWORD",  # exclude when description contains this keyword
],
```
